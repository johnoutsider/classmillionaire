...
export default function SetsListClient({
  sets,
  students,
}: {
  sets: SetRow[];
        ...
        {sets.map((s) => (
          ...
          >
            ...
            <div className="text-right shrink-0">
              <p className="text-sm text-white font-medium">{s.questionCount} questions</p>
              <p className="text-xs text-blue-500">{new Date(s.updatedAt).toLocaleDateString()}</p>
            </div>
          </div>
        ))}
      </div>
